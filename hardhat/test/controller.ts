...
describe("Controller", () => {
    ...
    async function checkDeal(storage: HiveStorage, party: string) {
        ...
        expect(await storage.getDealsForParty(getAddress(party))).to.deep.equal([
            DEAL_ID,
        ]);
    }

    async function checkAgreement(storage: HiveStorage, desiredState: string) {
        const agreement = await storage.getAgreement(DEAL_ID);
        expect(agreement.state).to.equal(getAgreementState(desiredState));
    }

    async function agree(controller: HiveController, party: string) {
        ...
        return controller
            ...
            .agree(DEAL_ID, members, timeouts, pricing);
    }

    async function setupController() {
        const {token, payments, storage, users, mediation, controller} =
    ...
    describe("Deals", () => {
        it("Should agreeResourceProvider", async function () {
            const {token, payments, storage, users, controller, tokenAddress} =
        ...
        it("Should agreeJobCreator", async function () {
            const {token, payments, storage, controller, tokenAddress} =
                await loadFixture(setupController);

            const balancesBeforeAgreeJC = await getBalances(token, "job_creator");

            await expect(agree(controller, "job_creator"))
                .to.emit(payments, "Payment")
                .withArgs(
                    DEAL_ID,
        ...
        it("Should agree both sides", async function () {
            const {storage, controller} = await loadFixture(setupController);

            await expect(agree(controller, "job_creator")).to.not.be.reverted;

            await expect(agree(controller, "resource_provider"))
                .to.emit(storage, "DealStateChange")
                .withArgs(DEAL_ID, getAgreementState("DealAgreed"));
    ...
    describe("Results", () => {
        it("Post results as RP", async function () {
            const {token, tokenAddress, payments, storage, controller} =
    ...
    describe("End to end", () => {
        it("Runs a job in the happy path", async function () {
            const {token, storage, controller} = await loadFixture(setupController);
            ...
            await controller
                .connect(getWallet("resource_provider"))
                .addResult(DEAL_ID, RESULTS_ID, DATA_ID, instructionCount);
            await controller.connect(getWallet("job_creator")).acceptResult(DEAL_ID);
        ...
        it("Runs a job in the mediation OK path", async function () {
            const {token, storage, users, controller, mediation} =
                await loadFixture(setupController);
            ...
            await controller
                .connect(getWallet("resource_provider"))
                .addResult(DEAL_ID, RESULTS_ID, DATA_ID, instructionCount);
            await controller.connect(getWallet("job_creator")).checkResult(DEAL_ID);
            await mediation
                .connect(getWallet("mediator"))
                .mediationAcceptResult(DEAL_ID);
        ...
        it("Runs a job in the mediation not OK path", async function () {
            const {token, storage, users, mediation, controller} =
                await loadFixture(setupController);
